...
| --- | --- | --- | --- | --- | --- | --- |
| R6-01 | FE | 사이드바 | 작성자 DB 항목 `adminOnly` 해제 (편집자에게 자물쇠 없이 열림) | `SidebarNav.tsx` | 헤드리스 1440px, `AUTH_DEV_ROLE=편집자`로 자물쇠 3개(데이터 대시보드, 질문지, 평가 DB)만 남는 것 확인 | ✅ |
| R6-02 | FE+BE | `/data/authors`, `/data/authors/[id]` | 관리자 게이트 제거. 편집자는 **용역비 열(목록)과 용역비 행(상세 평가 패널)만 숨김**, 관리자는 그대로. 숨김은 화면만이 아니라 **페이로드에서도** 뺀다: 목록 행·작성자·평가의 `scholarship_amount`, "용역비 등급 …" 평가 요약. 로컬 확인용 `AUTH_DEV_ROLE=편집자` 스위치 추가(`auth.ts`, dev bypass 전용) | `data/authors/page.tsx`, `data/authors/[authorId]/page.tsx`, `EvaluationPanel.tsx`, `auth.ts` | 편집자 역할 헤드리스: 목록 헤더에 용역비 없음, 상세 제목 "평가", 본문에 "용역비" 0회, 표 props에 금액 0회. dev 모드 페이로드에는 React 개발용 I/O 추적(D1 결과 원본)이 남지만 `npm run preview`(프로덕션 빌드, :8787)에서는 `scholarship_amount` 0회, 용역비 0회 확인 | ✅ |
| R6-03 | FE+DOC | 잠금 안내 | 편집자 사용 범위 문구를 "원고 관리, 수기 DB, 작성자 DB"로 갱신. README 역할 설명 동일 갱신 | `Locked.tsx`, `README.md` | 편집자 역할로 `/data` 잠금 문구 확인 | ✅ |
| R6-04 | FE | 셸 | (폐기 → R6-12) 모바일 햄버거 + 오버레이 드로어. 실기기에서 스크롤 중 드로어가 잘려 보이는 문제와 "사이드바 대신 아이콘/BNB" 요청으로 같은 회차에 교체 | — | — | — |
| R6-05 | FE | 전 화면 | 모바일 여백: 페이지 헤더·본문·히어로·단계 바 좌우 28px → 16px | `globals.css` | 헤드리스 390px 스크린샷 | ✅ |
| R6-06 | FE | 표 | 모바일에서 `.table-wrap`만 가로 스크롤(표 최소 폭 640px), 페이지 가로 넘침 없음. 2열 grid(`dash-grid`, `detail-grid` 등)는 `1fr`이 아니라 `minmax(0,1fr)`로 접어야 표 최소 폭이 열을 밀지 않는다 | `globals.css` | 헤드리스 390px: 11개 화면 모두 `scrollWidth` 390 (변경 전 415~994) | ✅ |
| R6-07 | FE | 필터바 | 드롭다운·검색창 고정 폭을 모바일에서 2열 유동 폭으로 덮어씀 | `globals.css` | 390px 작성자 DB, 수기 DB 스크린샷 | ✅ |
| R6-08 | FE | 모달 | 모바일 전면 모달(높이 100dvh, radius 0), 넓은 모달 동일 | `globals.css` | 390px 목차 미리보기 모달 rect (0,0,390,844), radius 0 | ✅ |
| R6-09 | FE | 그리드 | KPI 4열 → 2열, 호차 카드 1열, 환경설정·마이페이지 1열, 목차 입력 행(`.toc-field-row`)·탈고 머리 행(`.rev-head-row`)의 인라인 grid를 클래스로 옮겨 2열/1열(데스크톱 값 동일) | `globals.css`, `TocInputEditor.tsx`, `RevisionEditor.tsx` | 390px 대시보드, 환경설정, 마이페이지 스크린샷. 탈고 편집기 화면은 미촬영 | ✅ |
| R6-10 | FE | 히어로 | 모바일 제목 28 → 22px, 표지 이미지 숨김, 메타 줄바꿈, 단계 줄은 가로 스크롤 | `globals.css` | 390px 호차 상세 스크린샷 | ✅ |
| R6-11 | FE | 터치 | 메뉴 항목 44px, 입력 글자 16px(iOS 확대 방지), 맨 위로 버튼 16px 오프셋 | `globals.css` | CSS 적용만 확인, 실기기 미검증 | ✅ |
| R6-12 | FE | 셸(모바일) | **하단 탭(BNB)** 56px + 홈 인디케이터 여백: 대시보드, 편집 중, 편집 완료, 수기 DB, 더보기. 현재 경로의 탭을 포인트 컬러로 표시(하위 경로는 상위 탭). 더보기는 BNB 위로 올라오는 **전체 메뉴 시트**로, 사이드바와 같은 `NAV`를 섹션째 보여주고(관리자 전용은 자물쇠) 계정 섹션(마이페이지, 로그아웃)을 덧붙인다. 경로 이동·배경 탭·X로 닫힘. 사이드바는 모바일에서 `display:none`, 상단바는 로고 + 마이페이지 아바타만 두고 고정을 풀었다. 본문 하단 여백과 맨 위로 버튼을 BNB 높이만큼 올림 | `MobileNav.tsx`(신규), `AppShell.tsx`, `SidebarNav.tsx`(NAV·isActive export), `globals.css` | 헤드리스 390px: 3000px 스크롤 후 BNB rect 고정, 시트 열림·항목 12개·환경설정 이동 시 자동 닫힘, 호차 상세에서 "편집 중" 탭 활성 | ✅ |
| R6-13 | FE | 셸(데스크톱) | 사용자 실브라우저에서 스크롤 시 사이드바가 따라 움직임(헤드리스 Chrome에서는 재현 안 됨, sticky + flex 조합). 사이드바를 `position: fixed`(top 0, left 0, bottom 0)로 바꾸고 `.main`에 `margin-left: var(--sidebar-w)`(접힘 56px, 전환 0.2s)를 줌. 모바일은 margin 0 | `globals.css` | 헤드리스 1440px: 스크롤 2500 후 사이드바 top 0, 접힘 시 main x 56, 390px 영향 없음. 픽셀 비교 baseline 동일 | ✅ |
| R6-14 | FE | BNB | 탭 아이콘 22 → 20px, 선 굵기 약 2px → 1.25px(래퍼가 16px 기준 절대 굵기라 확대 시 굵어지던 것) | `globals.css` | 390px 캡처 | ✅ |
| G-5 | 회귀 | 1440px | 변경 전후 헤드리스 풀페이지 스크린샷 픽셀 비교(11개 화면). BNB 교체 후 재비교: 8개 동일, `/data`는 표 헤더 글자 안티에일리어싱 2,620px, `/ax`는 좌하단 Next dev 배지(N) 유무 3,479px, `/ax/issues/[id]`는 1px — 레이아웃 차이 없음. **주의**: 풀페이지 캡처 중 뷰포트 에뮬레이션이 바뀌며 사이드바 transition이 찍히므로 `animations="disabled"`로 찍어야 한다 | — | 픽셀 비교 | ✅ |
...
| R5-10 | DATA | 2027 1호차 | (기능과 무관한 데이터 채우기) 러너 중지·큐 정리 후 **게재 원고 48편을 목차별 확정 수기로 적재**(`scripts/seed/2027-1_manuscripts.sql`). 원문은 작성자 답변 중 유사도 최고값을 연결(articles.qna_id도 채움). 17편은 유사도 0.2 미만(대부분 Part 4 콘텐츠 추천 — 여러 답변 합성 원고). 목차 6개 S1~S3 완료 처리(confirmed_by '데이터 적재'). 데모 원고 3편은 Part 2 Ch1에 남아 있어 그 목차만 9편 | 시드 SQL, 로컬·원격 D1 | 호차 화면 3단계 현재, 원고 51편 | ✅ |
